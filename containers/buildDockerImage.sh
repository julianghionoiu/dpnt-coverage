...
BASE_IMAGE_VERSION=$( cat "${SCRIPT_CURRENT_DIR}/base/version.txt" | tr -d " " | tr -d "\n" )
...
docker build -t ${BASE_IMAGE_TAG} "${SCRIPT_CURRENT_DIR}/base/."
...
docker build -t ${language_image_tag} --build-arg BASE_IMAGE="${BASE_IMAGE_TAG}" \
    "${SCRIPT_CURRENT_DIR}/${LANGUAGE_ID}/."